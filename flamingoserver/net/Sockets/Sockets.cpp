...
SOCKET sockets::createOrDie()
{
    ...
    if (sockfd == INVALID_SOCKET)
    {
        LOG_ERROR("sockets::createOrDie(),socket()出错");
    }
#endif
...
SOCKET sockets::createNonblockingOrDie()
{
    ...
    if (sockfd == INVALID_SOCKET)
    {
        LOG_ERROR("sockets::createNonblockingOrDie(),里 socket()出错");
    }
#endif
...
void sockets::bindOrDie(SOCKET sockfd, const struct sockaddr_in &addr)
{
    ...
    if (ret == SOCKET_ERROR)
    {
        LOG_ERROR("sockets::bindOrDie, 里 bind()出错 ");
    }
}
...
void sockets::listenOrDie(SOCKET sockfd)
{
    ...
    if (ret == SOCKET_ERROR)
    {
        LOG_ERROR("sockets::listenOrDie, listen()出错");
    }
}
...
SOCKET sockets::accept(SOCKET sockfd, struct sockaddr_in *addr)
{
    ...
    if (connfd == SOCKET_ERROR)
    {
        ...
        case EOPNOTSUPP:
            // unexpected errors
            LOG_ERROR("Socket::accept, unexpected error of ::accept %d", savedErrno);
            break;
        default:
            LOG_ERROR("Socket::accept, unknown error of ::accept %d", savedErrno);
            break;
        }
...
void sockets::setReusePort(SOCKET sockfd, bool on)
{
    ...
    if (ret < 0 && on)
    {
        LOG_ERROR("sockets::setReusePort, setsockopt()调用失败.");
    }
#endif
}
...
void sockets::toIpPort(char *buf, size_t size, const struct sockaddr_in &addr)
{
    ...
    // LOG_DEBUG_WS("std::string 2  = %s", buf);
}

void sockets::toIp(char *buf, size_t size, const struct sockaddr_in &addr)
{
    // cout << "std::string 3 = "
    //      << "size = " << size << "sizeof(struct sockaddr_in) == " << sizeof(struct sockaddr_in) << endl;
    // if (size >= sizeof(struct sockaddr_in))
    //     return;

    ::inet_ntop(AF_INET, &addr.sin_addr, buf, static_cast<socklen_t>(size));
    // LOG_DEBUG_WS("std::string 1  = %s", buf);
}
