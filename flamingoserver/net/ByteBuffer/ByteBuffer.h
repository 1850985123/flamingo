...
namespace net
{
    ...
    {
    public:
        static const size_t kCheapPrepend = 8; // deng: 预留不用的空间，实际没有使用，可能以后会用到吧？ 感觉没必要存在。
        static const size_t kInitialSize = 1024; //deng; 要使用的空间
        ...
        //deng: 返回还有多少空间可以写入（到缓冲区）。
        size_t writableBytes() const
        {
            return m_buffer.size() - m_writerIndex;
        }

        //deng; 已经被读取了内容的，空闲下来的空间
        size_t prependableBytes() const
        {
            return m_readerIndex;
        }

        //deng: 查看缓冲区的位置，但缓冲区的读位置不会改变
        ...
        //deng: 这个操作如果容器的空间不够的话会自动扩容。
        void append(const char* /*restrict*/ data, size_t len)
        {
            //其实相当于把已有数据往前挪动
            ensureWritableBytes(len);
            std::copy(data, data + len, beginWrite());
            hasWritten(len); // deng: 修改写的位置
        }
    ...
    private:
        ...
        void makeSpace(size_t len)
        {
            ...
            else
            {
                ...
                //degn: 把还没有被客户读取出去的数据前移， 这个操作感觉有点效率低、先不评价好不好。
    ...
    private:
        ...
        size_t                      m_readerIndex;   // 当前读的位置  
        size_t                      m_writerIndex;  //当前写的位置
