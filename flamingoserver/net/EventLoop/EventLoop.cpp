    ...
    currentActiveChannel_(NULL)
{
    ...
    // dneg；一个线程只会有一个 EventLoop，否则程序出错
    if (t_loopInThisThread)
    {
        LOG_ERROR("EventLoop::EventLoop, 本线程已经创建过 EventLoop 了");
...
EventLoop::~EventLoop()
{
    ...
    LOG_DEBUG("EventLoop::~EventLoop, EventLoop 0x%x destructs.", this);
...
void EventLoop::loop()
{
    ...
    LOG_DEBUG("EventLoop::loop() , EventLoop = 0x%x  start looping", this);
    ...
    // LOG_DEBUG("EventLoop::loop() , EventLoop 0x%0x stop looping , threadID = %lu", this, std::this_thread::get_id());
    m_looping = false;

    std::ostringstream oss;
    oss << std::this_thread::get_id();
    std::string stid = oss.str();
    LOG_DEBUG("EventLoop::loop() , EventLoop 0x%0x stop looping , threadID = %lu", this, stid.c_str());
}
...
void EventLoop::quit()
{
    ...
    if (!isInLoopThread())
    {
        //deng: 应为线程可能阻塞在 epoll_wait上，所以需要触发一个事件，退出epoll_wait函数。
        wakeup();
    }
}
...
void EventLoop::runInLoop(const Functor& cb)
{
    ...
    else //deng: 如果是其他线程调用，把回调函数放在 等待被执行的线程的 任务队列 中
    {
        queueInLoop(cb);
    }
}
...
bool EventLoop::createWakeupfd()
{
    ...
    {
        //让程序挂掉
        LOG_ERROR("EventLoop::createWakeupfd(), Unable to create wakeup eventfd, EventLoop: 0x%x", this);
        return false;
    }
...
void EventLoop::abortNotInLoopThread()
{
    ...
    LOG_ERROR("EventLoop::abortNotInLoopThread - EventLoop %s", ss.str().c_str());
}
...
#else
        int error = errno;
        LOG_ERROR("EventLoop::wakeup() writes %d  bytes instead of 8, fd: %d, error: %d, errorinfo: %s", n, m_wakeupFd, error, strerror(error));
#endif
...
#else
        int error = errno;
        LOG_ERROR("EventLoop::wakeup() read %d  bytes instead of 8, fd: %d, error: %d, errorinfo: %s", n, m_wakeupFd, error, strerror(error));
#endif
        return false;
    }
