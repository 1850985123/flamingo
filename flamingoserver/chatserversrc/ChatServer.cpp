...
void ChatServer::onConnected(std::shared_ptr<TcpConnection> conn)
{
    if (conn->connected())
    {
        ...
        LOG_DEBUG("ChatServer::onConnected 增加客户端连接，client = %s, 当前 session 数量 = %d", conn->peerAddress().toIpPort().c_str(), m_sessions.size());
    }
    else
    {
        onDisconnected(conn);
...
void ChatServer::onDisconnected(const std::shared_ptr<TcpConnection>& conn)
{
    ...
    for (auto iter = m_sessions.begin(); iter != m_sessions.end(); ++iter)
    {
        ...
        if ((*iter)->getConnectionPtr() == conn)
        {
            ...
            // m_sessionId--;//deng: add 2022_7_9;
            LOG_DEBUG("ChatServer::onDisconnected 断开 客户端连接，clietn = %s, 当前 session 数量 = %d", conn->peerAddress().toIpPort().c_str(), m_sessions.size());
