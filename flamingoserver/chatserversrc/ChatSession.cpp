    ...
    while (true)
    {
        ...
        if (header.compressflag == PACKAGE_COMPRESSED)
        {
                ...
                header.originsize <= 0 || header.originsize > MAX_PACKAGE_SIZE)
            {
                ...
                LOG_DEBUG("数据压缩, ChatSession::onRead() 非法数据包, compresssize = %lld, originsize = %lld, 关闭 TcpConnection, client: %s",
            ...
            {
                LOGE("数据压缩, ChatSession::onRead()里 ZlibUtil::uncompressBuf() error, 关闭 TcpConnection, client = %s", conn->peerAddress().toIpPort().c_str());
                // LOGE("uncompress error, client: %s", conn->peerAddress().toIpPort().c_str());
                conn->forceClose();
                return;
            }

            // deng: 接收完一条报文，接下来对业务的处理
            if (!process(conn, destbuf.c_str(), destbuf.length()))
            {
                ...
                LOG_DEBUG("数据压缩, ChatSession::onRead()里 process() error, 关闭 TcpConnection, client = %s", conn->peerAddress().toIpPort().c_str());
                conn->forceClose();
                return;
            }
        ...
        else
        {
            ...
            {
                //客户端发非法数据包，服务器主动关闭之
                LOG_DEBUG("数据未压缩, ChatSession::onRead() 非法数据包, compresssize = %lld, originsize = %lld, 关闭 TcpConnection, client: %s", header.compresssize, header.originsize, conn->peerAddress().toIpPort().c_str());
                conn->forceClose();
                return;
            }
            ...
            if (!process(conn, inbuf.c_str(), inbuf.length()))
            {
                //客户端发非法数据包，服务器主动关闭之
                // LOGE("Process error, close TcpConnection, client: %s", conn->peerAddress().toIpPort().c_str());
                LOG_DEBUG("数据未压缩, ChatSession::onRead()里 process() error, 关闭 TcpConnection, client = %s", conn->peerAddress().toIpPort().c_str());
                conn->forceClose();
                return;
            }
...
bool ChatSession::process(const std::shared_ptr<TcpConnection> &conn, const char *inbuf, size_t buflength)
{
    ...
    {
        LOG_DEBUG("接收到客户端消息 client = %s", conn->peerAddress().toIpPort().c_str());
        LOG_DEBUG("消息内容：cmd=%d, seq=%d, data=%s, datalength=%d, buflength=%d", cmd, m_seq, data.c_str(), datalength, buflength);
    }
    ...
    default:
    {
        if (m_isLogin)
        {
            ...
            case msg_type_chat:
            {
                ...
                LOG_DEBUG_WS("接收到聊天消息 = %s", data.c_str());
            ...
            case msg_type_movefriendtootherteam:
            {
                ...
                LOG_DEBUG_WS("移动好友到新分组 = %s", newteamname.c_str());
...
/*  dneg: 登录需要信息 用户名 + 密码 + 客户端类型 + status;
    一个账号只能有一个客户端在线。当别的客户端上线后需要把之前的客户端体下线。
...
void ChatSession::onLoginResponse(const std::string &data, const std::shared_ptr<TcpConnection> &conn)
{
    ...
    if (cachedUser.userid == 0)
    {
        ...
        LOG_DEBUG("用户登录失败， 还没注册， client = %s", conn->peerAddress().toIpPort().c_str());
    }
    else
    {
        if (cachedUser.password != password)
        {
            ...
            LOG_DEBUG("用户登录密码错误， client = %s", conn->peerAddress().toIpPort().c_str());
        }
    ...
    //设置已经登录的标志
    m_isLogin = true;
    LOG_DEBUG("用户登录成功， client = %s", conn->peerAddress().toIpPort().c_str());
    LOG_DEBUG("用户信息： userid = %d, username =  %s, password = %s, nickname: %s",
...
// deng 如果状态和之前一样不用管， 不一致则更新状态并且通知好友状态改变了。
...
// deng: 用户信息改变后， 修改数据库用户数据、修改用户缓存数据，并且给好友发送信息改变的消息。
...
void ChatSession::onModifyPasswordResponse(const std::string &data, const std::shared_ptr<TcpConnection> &conn)
{
    ...
    string retdata;
    User cachedUser;
    // deng: 用userid查找的都可以username查找， 不过查找整形数据应该会比字符串快吧？ 暂时先不思考这个问题
...
void ChatSession::onUpdateTeamInfoResponse(int32_t operationType, const std::string &newTeamName, const std::string &oldTeamName, const std::shared_ptr<TcpConnection> &conn)
{
    ...
    LOG_DEBUG_WS("新分组信息 = %s", newTeamInfo.c_str());
...
void ChatSession::deleteFriend(const std::shared_ptr<TcpConnection> &conn, int32_t friendid)
{
    ...
    **/
    ...
    // deng: releaseFriendRelationshipInDBAndMemory 已经操作过了，updateUserRelationshipInMemory 这个操作多余了啊！！！
    ...
    //发给主动删除的一方
    //{"userid": 9, "type": 1, }
    // deng: 删除都不用 username 为什么应答删除成功却需要，感觉协议制定有问题，多此一举的感觉。
    ...
    for (const auto &iter : friends)
    {
        ...
        if (!targetSessions.empty())
        {
            for (auto &iter2 : targetSessions)
            {
                // deng; friendid 表明是哪个群
                if (iter2)
                    iter2->sendUserStatusChangeMsg(friendid, 3); // deng: 这个退群推送消息感觉不太细节、有点问题，比如谁退出了群，
            }
        }
    }
}
